		...
		if (isset($filter)) {
			...
			for ($i=0; $i < count($partesDoFiltro); $i++) { 
				...
				if ($condicoes[1] == '$cont') {
					...
					$this->where = $this->where . " $this->campo like '%" . $this->valor . "%'";
				}
				...
				if ($condicoes[1] == '$eq') {
					...
					$this->where = $this->where . " $this->campo = '" . $this->valor . "'";
				}
				...
				if ($condicoes[1] == '$between') {
					...
					$this->where = $this->where . " $this->campo between '" . $this->dataInicial . "' and '" . $this->dataFinal . "'";
				}

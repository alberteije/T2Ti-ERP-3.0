		...
		if (isset($filter)) {
			...
			for ($i=0; $i < count($partesDoFiltro); $i++) { 
				...
				if ($condicoes[1] == '$cont') {
					...
					$this->where = $this->where . "t.$campoPesquisa like '%" . $this->valor . "%'";
				}
				...
				if ($condicoes[1] == '$eq') {
					...
					$campoPesquisa = Biblioteca::camelCase($this->campo);
					$this->where = $this->where . "t.$campoPesquisa = '" . $this->valor . "'";
				}
				...
				if ($condicoes[1] == '$between') {
					...
					$campoPesquisa = Biblioteca::camelCase($this->campo);
					$this->where = $this->where . "t.$campoPesquisa between '" . $this->dataInicial . "' and '" . $this->dataFinal . "'";
				}

...
begin
  try
    ...
    ListaProduto := TListaViewSpedNfeItemVO(TT2TiORM.ConsultarLista(TViewSpedNfeItemVO.Create, 'ID in (select ID_PRODUTO from NFE_DETALHE JOIN NFE_CABECALHO ON NFE_DETALHE.ID_NFE_CABECALHO = NFE_CABECALHO.ID' + ' where NFE_CABECALHO.DATA_HORA_EMISSAO BETWEEN ' + QuotedStr(DataInicial) + ' and ' + QuotedStr(DataFinal) + ')'));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //ListaNF2Cabecalho := TT2TiORM.ConsultarLista(TEcfNotaFiscalCabecalhoVO.Create, 'DATA_EMISSAO BETWEEN ' + QuotedStr(DataInicial) + ' and ' + QuotedStr(DataFinal));
        //ListaNF2CabecalhoCanceladas := TT2TiORM.ConsultarLista(TEcfNotaFiscalCabecalhoVO.Create, 'CANCELADA = ' + QuotedStr('S') + ' and DATA_EMISSAO BETWEEN ' + QuotedStr(DataInicial) + ' and ' + QuotedStr(DataFinal));
        ...
        begin
          ...
          //ListaC300 := TT2TiORM.ConsultarLista(TViewSpedC300VO, 'DATA_EMISSAO BETWEEN ' + QuotedStr(DataInicial) + ' and ' + QuotedStr(DataFinal));
          ...
          //ListaImpressora := TT2TiORM.ConsultarLista(TEcfImpressoraVO, 'ID>0');
          ...
          //        ListaR02 := TT2TiORM.ConsultarLista(TEcfR02VO, FiltroLocal);
          ...
          //                ListaC425 := TT2TiORM.ConsultarLista(TViewSpedC425VO, FiltroLocal);
...
begin
  try
    ...
    begin
      // REGISTRO I001: ABERTURA DO BLOOC I
...
begin
  ...
  // BLOCO I: OPERAÇÕES DAS INSTITUIÇÕES FINANCEIRAS, SEGURADORAS, ENTIDADES DE PREVIDENCIA PRIVADA
  GerarBlocoI;
  ...
  Arquivo := 'SpedContribuicoes' + FormatDateTime('DDMMYYYYhhmmss', Now) + '.txt';

  FDataModule.ACBrSpedContribuicoes.Path := ExtractFilePath(Application.ExeName) + 'Arquivos\Sped\';
  FDataModule.ACBrSpedContribuicoes.Arquivo := Arquivo;
  FDataModule.ACBrSpedContribuicoes.SaveFileTXT;

  Arquivo := FDataModule.ACBrSpedContribuicoes.Path + Arquivo;
  Result := True;
end;
{$ENDREGION}

...
export class FlutterPersistePageBase {
                ...
                if (Biblioteca.isJsonString(this.table, this.campoModel.Comment)) {
                    try {
                        ...
                        if (this.objetoJsonComentario.desenhaControle == null || this.objetoJsonComentario.desenhaControle == true) {
                            ...
                            // no arquivo de template os lookups vem sempre na frente dos demais campos.
                            // com os comentários bootstrap no der não há necessidade de criar inputs "normais" e "lockups".
                            // define todos os inputs como "normais", pois a ordem está definida nas linhas e colunas bootstrap
                            if (this.campoSendoMontadoEhLookup) {
                                for (let j = 0; j < this.inputLookup.length; j++) {
                                    this.inputNormal.push(this.inputLookup[j]);
                                }
                                this.inputLookup = [];
                            }
                        }
    ...
    montarCampoComLookup(paginaDeAba: boolean, controlaFormAlteradoLocal: boolean) {
        ...
        if (this.campoModel.nomeCampoTabela.includes('ID_')) {
            ...
            this.inputLookup.push("                    widget." + this.objetoPrincipal + "?." + this.objetoJsonComentario.objetoMestre + " = " + this.objetoJsonComentario.classeMestre + ".fromJson(objetoJsonRetorno);");
        } else { // se é um campo de lookup para simples consulta, não precisa vincular ao objeto mestre, pois o dado será persistido no campo da própria tabela
    ...
    abrirCampoDeLookup() {
        ...
        this.inputLookup.push("            child: SizedBox(");
    ...
    abrirCampoDeLookupExpanded() {
        ...
        this.inputLookup.push("                          valorPesquisaPadrao: '" + (this.objetoJsonComentario.valorPadraoLookup != null ? this.objetoJsonComentario.valorPadraoLookup : "") + "',");
        this.inputLookup.push("                        ),");
    ...
    montarTextFormFieldPadrao(paginaDeAba: boolean, controlaFormAlteradoLocal: boolean) {
        ...
        if (this.objetoJsonComentario.tipoControle.mascara != '' && this.objetoJsonComentario.tipoControle.mascara != null) {
            ...
            this.inputNormal.push(this.completaEspacosFilhoPadding + "        controller: " + this.campoModel.nomeCampoAtributo + "Controller,");
        ...
        if (this.campoModel.tipoCampo.includes('decimal')) {
            this.inputNormal.push(this.completaEspacosFilhoPadding + "          widget." + this.objetoPrincipal + "?." + this.campoModel.nomeCampoAtributo + " = " + this.campoModel.nomeCampoAtributo + "Controller.numberValue;");
    ...
    montarDatePickerItem(paginaDeAba: boolean) {
        ...
        this.inputNormal.push(this.completaEspacosFilhoPadding + "          mascara: '" + this.objetoJsonComentario.tipoControle.mascara + "',");
